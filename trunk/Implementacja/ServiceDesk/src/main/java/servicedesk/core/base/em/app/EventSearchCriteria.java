...
public class EventSearchCriteria implements SearchCriteria<Event> {
    ...
    public void initCriteria(Criteria criteria) {
        ...
        if (significance != null) {
            criteria.add(Restrictions.eq("significance", significance));
        }
        if (category != null) {
            criteria.add(Restrictions.eq("category", category));
        }
        if (startCreationDate != null) {
            criteria.add(Restrictions.ge("creationDate", startCreationDate));
        }
        if (endCreationDate != null) {
            criteria.add(Restrictions.le("creationDate", endCreationDate));
        }
        if (startEventDate != null) {
            criteria.add(Restrictions.ge("eventDate", startEventDate));
        }
        if (endEventDate != null) {
            criteria.add(Restrictions.le("eventDate", endEventDate));
        }
    }
