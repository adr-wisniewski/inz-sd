...
ALTER TABLE SD.INCIDENTS 
DROP CONSTRAINT INCIDENTS_EMPLOYEES_FK2;
...
ALTER TABLE SD.RFC_CATEGORIES 
DROP CONSTRAINT RFC_CATEGORY_RFC_CATEGORY_FK1;

ALTER TABLE SD.RFC 
DROP CONSTRAINT RFC_AUTHORITY_FK1;

ALTER TABLE SD.RFC 
DROP CONSTRAINT RFC_EMPLOYEES_FK1;

ALTER TABLE SD.RFC 
DROP CONSTRAINT RFC_EMPLOYEES_FK2;

ALTER TABLE SD.RFC 
DROP CONSTRAINT RFC_RFC_CATEGORY_FK1;
...
ALTER TABLE SD.RFC 
DROP CONSTRAINT RFC_RFC_RESOLUTIONS_FK1;

ALTER TABLE SD.AUTHORITY_MEMBERS 
DROP CONSTRAINT CAB_MEMBERS_CAB_FK1;

ALTER TABLE SD.AUTHORITY_MEMBERS 
DROP CONSTRAINT CAB_MEMBERS_EMPLOYEES_FK1;

ALTER TABLE SD.AUTHORITIES 
DROP CONSTRAINT AUTHORITY_AUTHORITY_TYPE_FK1;

ALTER TABLE SD.AUTHORITIES 
DROP CONSTRAINT AUTHORITY_RFC_CATEGORY_FK1;

ALTER TABLE SD.AUDITREVISION 
DROP CONSTRAINT AUDITREVISION_EMPLOYEES_FK1;
...
DROP SEQUENCE SD.RFC_RESOLUTIONS_SEQ;

DROP SEQUENCE SD.RFC_PRIORITIES_SEQ;

DROP SEQUENCE SD.RFC_IMPACTS_SEQ;

DROP SEQUENCE SD.RFC_CATEGORIES_SEQ;

DROP SEQUENCE SD.AUTHORITY_TYPES_SEQ;

DROP SEQUENCE SD.AUTHORITIES_SEQ;

DROP SEQUENCE SD.AUDITREVISION_SEQ;
...
DROP TABLE SD.RFC_RESOLUTIONS CASCADE CONSTRAINTS;

DROP TABLE SD.RFC_PRIORITIES CASCADE CONSTRAINTS;

DROP TABLE SD.RFC_IMPACTS CASCADE CONSTRAINTS;

DROP TABLE SD.RFC_CATEGORIES CASCADE CONSTRAINTS;

DROP TABLE SD.RFC CASCADE CONSTRAINTS;

DROP TABLE SD.AUTHORITY_TYPES_MATRIX CASCADE CONSTRAINTS;

DROP TABLE SD.AUTHORITY_TYPES CASCADE CONSTRAINTS;

DROP TABLE SD.AUTHORITY_MEMBERS CASCADE CONSTRAINTS;

DROP TABLE SD.AUTHORITIES CASCADE CONSTRAINTS;

DROP TABLE SD.RFC_AUD CASCADE CONSTRAINTS;

DROP TABLE SD.AUDITREVISION CASCADE CONSTRAINTS;
...
CREATE TABLE SD.RFC_RESOLUTIONS 
(
  RESOLUTION_ID NUMBER(10) NOT NULL 
, NAME VARCHAR2(200) NOT NULL 
, "ORDER" NUMBER(10) NOT NULL 
, CONSTRAINT RFC_RESOLUTIONS_PK PRIMARY KEY 
  (
    RESOLUTION_ID 
  )
  ENABLE 
...
CREATE TABLE SD.RFC_PRIORITIES 
(
...
CREATE TABLE SD.RFC_CATEGORIES 
(
...
, CONSTRAINT RFC_CATEGORY_PK PRIMARY KEY 
  (
    CATEGORY_ID 
  )
  ENABLE 
...
CREATE TABLE SD.AUTHORITY_TYPES_MATRIX 
(
  PRIORITY_ID NUMBER(10) NOT NULL 
, IMPACT_ID NUMBER(10) NOT NULL 
...
CREATE TABLE SD.AUTHORITY_TYPES 
(
  TYPE_ID NUMBER(10) NOT NULL 
, NAME VARCHAR2(200) NOT NULL 
, IMPORTANCE NUMBER(10) NOT NULL 
, CONSTRAINT AUTHORITY_TYPE_PK PRIMARY KEY 
  (
    TYPE_ID 
  )
  ENABLE 
...
CREATE TABLE SD.AUTHORITY_MEMBERS 
(
  AUTHORITY_ID NUMBER(10) NOT NULL 
, EMPLOYEE_ID NUMBER(10) NOT NULL 
, CONSTRAINT CAB_MEMBERS_PK PRIMARY KEY 
  (
    AUTHORITY_ID 
  , EMPLOYEE_ID 
  )
...
CREATE TABLE SD.AUTHORITIES 
(
  AUTHORITY_ID NUMBER(10) NOT NULL 
, CATEGORY_ID NUMBER(10) NOT NULL 
, TYPE_ID NUMBER(10) NOT NULL 
, EMERGENCY CHAR(1 BYTE) NOT NULL 
, CONSTRAINT CAB_PK PRIMARY KEY 
  (
    AUTHORITY_ID 
  )
  ENABLE 
...
, CATEGORY_ID NUMBER(10) 
, RESOLUTION_ID NUMBER(10) 
, CONSTRAINT RFC_AUD_PK PRIMARY KEY 
  (
    RFC_ID 
  , REV 
  )
...
CREATE TABLE SD.AUDITREVISION 
(
  ID NUMBER(10, 0) NOT NULL 
, TIMESTAMP TIMESTAMP NOT NULL 
, INSTIGATOR NUMBER(10) NOT NULL 
, CONSTRAINT AUDITREVISION_PK PRIMARY KEY 
  (
    ID 
  )
  ENABLE 
...
ALTER TABLE SD.RFC_RESOLUTIONS
ADD CONSTRAINT RFC_RESOLUTIONS_UK1 UNIQUE 
(
  NAME 
)
ENABLE;

ALTER TABLE SD.RFC_PRIORITIES
ADD CONSTRAINT RFC_PRIORITY_UK1 UNIQUE 
(
  NAME 
)
ENABLE;

ALTER TABLE SD.RFC_IMPACTS
ADD CONSTRAINT RFC_IMPACT_UK1 UNIQUE 
(
  NAME 
)
ENABLE;
...
ALTER TABLE SD.RFC_CATEGORIES
ADD CONSTRAINT RFC_CATEGORY_UK1 UNIQUE 
(
  NAME 
)
ENABLE;
...
ALTER TABLE SD.RFC_CATEGORIES
ADD CONSTRAINT RFC_CATEGORY_CHK1 CHECK 
(ABSTRACT IN ('Y', 'N'))
ENABLE;

ALTER TABLE SD.AUTHORITIES
ADD CONSTRAINT CAB_CHK1 CHECK 
(EMERGENCY IN ('Y', 'N'))
ENABLE;
...
COMMENT ON TABLE SD.USER_ROLE IS 'Tabela asocjacyjna zawieraj¹ca role przypisane dla danego pracownika';

COMMENT ON TABLE SD.ROLES IS 'S³ownik z mo¿liwymi rolami dla u¿ytkowników aplikacji.';

COMMENT ON TABLE SD.URGENCY_TYPES_PM IS 'S³ownik z typami pilnoœci dla problemów.';

COMMENT ON TABLE SD.URGENCY_TYPES_IM IS 'S³ownik z typami pilnoœci dla incydentów.';

COMMENT ON TABLE SD.SUPPORT_GROUPS IS 'Tabela z grupami wsparcia';

COMMENT ON TABLE SD.STATUSES_RF IS 'S³ownik z mo¿liwymi typami statusów dla zg³oszeñ.';

COMMENT ON TABLE SD.STATUSES_PM IS 'S³ownik z typami statusów dla problemów.';

COMMENT ON TABLE SD.STATUSES_IM IS 'S³ownik z mo¿liowymi statusami dla incydentów.';

COMMENT ON TABLE SD.SOLUTION_TYPES IS 'S³ownik z typami rozwi¹zañ. Mo¿e to byæ rozwi¹zanie sta³e jak i tymczasowe.';

COMMENT ON TABLE SD.SOLUTION_CATEGORIES IS 'S³ownik z typami kategorii rozwi¹zañ.';

COMMENT ON TABLE SD.SOLUTIONS IS 'Tabela zawieraj¹ca rozwi¹zania problemów.  S¹ to zarówno rozwi¹zania tymczasowe oraz rozwi¹zania sta³e.';

COMMENT ON TABLE SD.SIGNAL_TYPES IS 'Tabela zawieraj¹ca dane o typach sygna³ów.';

COMMENT ON TABLE SD.SIGNAL_SIGNIFICANCE_TYPES IS 'S³ownik mo¿liwych rang sygna³ów. Przyk³adowe rangi: informacja, ostrze¿enie, b³¹d.';

COMMENT ON TABLE SD.SIGNAL_RECEIVER_TYPES IS 'S³ownik mo¿liwych odbiorców sygna³ów.';

COMMENT ON TABLE SD.SIGNAL_OBJECT_TYPES IS 'Tabele zawieraj¹ce dane o typach obiektów. Przyk³adowe typy to: incydent, problem, zg³oszenie.';

COMMENT ON TABLE SD.SIGNAL_GENERATORS IS 'Zawiera informacje o generatorach sygna³ów.';

COMMENT ON TABLE SD.SIGNAL_ATTRIBUTES IS 'Zawiera informacje o wartoœciach atrybutów sygna³ów.';

COMMENT ON TABLE SD.SIGNALS_TMP IS 'Tabele tymczasowa przechowywuj¹ca dane o sygna³ach, które zostan¹ wygenerowany w zaistnieniu zdarzenia zmiany stanu. Trigger na poziomie wiersza wstawia dane do tej tabeli, natomiast trigger na poziomie tabeli pobiera wiersze z tej tabeli i wstawia do tabeli Signals.';

COMMENT ON TABLE SD.SIGNALS IS 'Zawiera informacje o sygna³ach.';

COMMENT ON TABLE SD.REQUEST_ATTRIBUTE_TYPES IS 'Tabela z typami atrybutów dla danej kateogrii zg³oszenia. Przyk³adowo je¿eli kategoria zg³oszenia jest to instalacja oprogramowania to typami atrybutów mog¹ byæ nazwa aplikacji oraz wersja aplikacji.';

COMMENT ON TABLE SD.REQUEST_ATTRIBUTES IS 'Tabela z wartoœciami atrybutów dla zg³oszeñ.';

COMMENT ON TABLE SD.REQUESTS IS 'Tabela z zg³oszeniami';

COMMENT ON TABLE SD.PROBLEMS IS 'Tabela z problemami';

COMMENT ON TABLE SD.PRIORITY_TYPES_RF IS 'S³ownik z mo¿liwymi typami priorytetów dla zg³oszeñ.';

COMMENT ON TABLE SD.PRIORITY_TYPES_PM IS 'S³ownik z typami priorytetów dla problemów.';

COMMENT ON TABLE SD.PRIORITY_TYPES_IM IS 'S³ownik z mo¿liwymi priorytetami dla incydentów.';

COMMENT ON TABLE SD.INCIDENT_PROBLEM IS 'Tabela asocjacyjna pomiêdzy problemami oraz incydentami.';

COMMENT ON TABLE SD.INCIDENTS_HISTORY IS 'Historia incydentów. Przy ka¿dej zmianie stanu incydentu wstawiany jest wiersz do tej tabeli.';

COMMENT ON TABLE SD.INCIDENTS IS 'Incydenty';

COMMENT ON TABLE SD.IMPACT_TYPES_PM IS 'S³ownik z typami wp³ywów dla problemów.';

COMMENT ON TABLE SD.IMPACT_TYPES_IM IS 'S³ownik z typami wp³ywów dla incydentów.';

COMMENT ON TABLE SD.GROUP_MEMBER IS 'Tabela asocjacyjna zawieraj¹ca listê cz³onków danej grupy wsparcia.';

COMMENT ON TABLE SD.EVENT_SIGNIFICANCE_TYPES IS 'S³ownik rang dla zdarzeñ. Przyk³adowe rangi to: informacje, ostrze¿enie, b³¹d.';

COMMENT ON TABLE SD.EVENT_PARAMETERS IS 'Lista wartoœci parametrów dla konkretnego zdarzenia.';

COMMENT ON TABLE SD.EVENT_INCIDENT IS 'Tabela asocjacyjna powi¹zuj¹ca zdarzenie z incydentem.';

COMMENT ON TABLE SD.EVENTS IS 'Zdarzenia';

COMMENT ON TABLE SD.COMMENTS IS 'Tabela z komentarzami';

COMMENT ON TABLE SD.CATEGORIES_RF IS 'S³ownik z mo¿liwymi typami kategorii dla zg³oszeñ.';

COMMENT ON TABLE SD.CATEGORIES_IM IS 'S³ownik z kategoriami incydentów';

COMMENT ON TABLE SD.CATEGORIES_EM IS 'S³ownik kategorii dla zdarzeñ.';

COMMENT ON TABLE SD.JOBS IS 'S³ownik z mo¿liwymi typami stanowisk.';

COMMENT ON TABLE SD.EMPLOYEES IS 'Tabela z pracownikami';

COMMENT ON TABLE SD.DEPARTMENTS IS 'S³ownik z departamentami.';

COMMENT ON COLUMN SD.SIGNAL_GENERATORS.TYPE_CODE IS 'Typ sygna³u';

COMMENT ON COLUMN SD.SIGNAL_GENERATORS.CRITERION IS 'Kryterium, które spe³niaj¹ obiekty';
...
COMMENT ON COLUMN SD.SIGNAL_GENERATORS.FREQUENCY IS 'Czêstotliwoœæ w minutach';
...
COMMENT ON COLUMN SD.PROBLEMS.IS_KNOWN_ERROR IS 'Czy to znany b³¹d';

COMMENT ON COLUMN SD.PROBLEMS.SUBJECT IS 'Temat problemu';

COMMENT ON COLUMN SD.PROBLEMS.DESCRIPTION IS 'Opis problemu';

COMMENT ON COLUMN SD.PROBLEMS.AUTHOR IS 'Pracownik, która doda³a problem';

COMMENT ON COLUMN SD.PROBLEMS.STAFF_MEMBER IS 'Serwisant do którego jest przypisany problem';

COMMENT ON COLUMN SD.PROBLEMS.GROUP_ID IS 'Grupa wsparcia do której jest przypisany problem';

COMMENT ON COLUMN SD.PRIORITY_TYPES_IM.TARGET_RESOLUTION_TIME IS 'Czas rozwi¹zania incydentu w godzinach.';

COMMENT ON COLUMN SD.COMMENTS.TEXT IS 'Treœæ komentarza';

COMMENT ON COLUMN SD.COMMENTS.CREATION_DATE IS 'Data dodania komentarza';

COMMENT ON COLUMN SD.COMMENTS.EMPLOYEE_ID IS 'Pracownik, który doda³ komentarz';
...
COMMENT ON COLUMN SD.CATEGORIES_IM.PARENT_CATEGORY IS 'Identyfikator kategorii nadrzêdnej';
...
CREATE SEQUENCE SD.RFC_RESOLUTIONS_SEQ START WITH 1000;

CREATE SEQUENCE SD.RFC_PRIORITIES_SEQ START WITH 1000;

CREATE SEQUENCE SD.RFC_IMPACTS_SEQ START WITH 1000;

CREATE SEQUENCE SD.RFC_CATEGORIES_SEQ START WITH 1000;

CREATE SEQUENCE SD.AUTHORITY_TYPES_SEQ;

CREATE SEQUENCE SD.AUTHORITIES_SEQ;

CREATE SEQUENCE SD.AUDITREVISION_SEQ;
...
BEGIN
	...
	INSERT INTO C2_ITEMS (ITEM_ID, LABEL)
        VALUES (C2_ENTITIES_SEQ.currval, 'US£UGA #' || to_char(:service.SERVICE_ID));
