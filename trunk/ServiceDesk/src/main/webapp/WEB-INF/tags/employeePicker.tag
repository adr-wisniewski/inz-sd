...
<%@ attribute name="value" required="true" type="sd.domain.Employee" %>
...
<a onclick="window.open('<c:url value="/sd/employeeDetails?id=${value.id}" />', 'Employee', 'menubar=no,toolbar=no,location=no,directories=no,status=no,scrollbars=yes,resizable=yes,fullscreen=no,channelmode=no').focus(); return false;" href="#" class="standard_link"><spring:message code="details.label" /></a>
</c:if>
