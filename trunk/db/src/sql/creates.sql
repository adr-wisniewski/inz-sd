...
DROP TABLE SD.EMPLOYEE_ROLE CASCADE CONSTRAINTS;
...
CREATE OR REPLACE VIEW SD.USERS AS SELECT
login as username
, password
, enabled
FROM employees;
...
CREATE OR REPLACE VIEW SD.AUTHORITIES AS SELECT 
 e.login as username
...
COMMENT ON TABLE SD.EMPLOYEE_ROLE IS 'Tabela asocjacyjna zawieraj¹ca role przypisane dla danego pracownika';
...
COMMENT ON COLUMN SD.EMPLOYEES.ENABLED IS 'Czy pracownik jest aktywny.';
